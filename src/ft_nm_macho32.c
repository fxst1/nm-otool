...
int				look_symtab_32(t_nm_otool *data, uint8_t *buf, t_freader *reader)
{
...
int			get_seg_sect_name_32(uint8_t *buf, size_t sect_index, t_symbol *s,
...
int				load_symbols_32(t_nm_otool *data, t_freader *reader, uint8_t *buf,
							uint8_t *start)
{
	size_t		i;
	size_t		sect_index;
	...
	while (i < reader->n_symbols)
	{
		...
		sect_index = *(buf + 5);
		if (sect_index > 0 &&
			get_seg_sect_name_32(start, sect_index, &s, reader->n_load_commands) != 0)
		{
